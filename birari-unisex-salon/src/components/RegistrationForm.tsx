      ...
      <div className="container-narrow py-10">
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-5">
          <div>
            ...
            <input id="name" type="text" autoComplete="name" value={name} onChange={(e) => setName(e.target.value)} className={`input-field ${errors.name ? "input-error" : ""}`} placeholder="e.g. Priya Sharma" aria-invalid={!!errors.name} aria-describedby={errors.name ? "name-error" : undefined} />
          ...
          <div>
            <label htmlFor="dob" className="label-text">Date of Birth <span className="normal-case text-black/30">(optional)</span></label>
            <input id="dob" type="date" value={dob} onChange={(e) => setDob(e.target.value)} max={new Date().toISOString().split("T")[0]} className={`input-field ${errors.dob ? "input-error" : ""}`} />
            {errors.dob && <p className="field-error-text">{errors.dob}</p>}
          </div>

          <div>
            <label htmlFor="service" className="label-text">Select Service</label>
            <select id="service" value={serviceId} onChange={(e) => setServiceId(e.target.value)} className={`input-field ${errors.serviceId ? "input-error" : ""}`} aria-invalid={!!errors.serviceId}>
          ...
          <div className="rounded-xl border border-black/10 bg-white px-4 py-3 text-sm text-black/60">
            Pre-booking amount: <span className="font-semibold text-ink">₹{prebookingAmount}</span>
          </div>

          {serverError && (
            <div className="flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
              <AlertIcon className="mt-0.5 h-4 w-4 shrink-0" /><span>{serverError}</span>
            </div>
          )}
          ...
          <Link href="/" className="block text-center text-xs font-medium text-black/40 hover:text-black/60">Back to home</Link>
        </form>
      </div>
    </main>
  );
